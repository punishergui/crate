...
export const concertsWidget = {
  ...
  render: (ctx) => {
    ...
    return {
      body: <ul className="activity-list scroll-list">{items.map((item, i) => <li key={`${item.artistName}-${item.date}-${i}`}><Artwork src={item.imageUrl || getArtistArtUrl(item)} alt={`${item.artistName} event`} fallbackSeed={item.artistName} size="sm" /><span><strong>{item.artistName}</strong> · {item.date} <span className="muted">{item.venue || 'Venue TBA'}</span></span></li>)}</ul>,
