  ...
  render: (ctx) => {
    ...
    return {
      body: <div className="discover-columns">
        <div><h4>New for You</h4>{(payload.newForYou || defaults.newForYou).slice(0, 3).map((item, i) => <AlbumTile key={`new-${i}`} album={item} subtext={item.artistName} size="sm" />)}</div>
        <div><h4>Weekly Picks</h4>{(payload.weeklyPicks || defaults.weeklyPicks).slice(0, 3).map((item, i) => <AlbumTile key={`weekly-${i}`} album={item} subtext={item.artistName} size="sm" />)}</div>
